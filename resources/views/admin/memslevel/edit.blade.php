	...
	<form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$memslevel->level_name}}" placeholder=""  name="level_name">
			</div>
		</div>
                <div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>消费金额：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$memslevel->amount}}" placeholder=""  name="amount">
			</div>
		</div>
                 <div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>折扣率：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$memslevel->discount}}" placeholder=""  name="discount">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
                            <textarea  class="textarea"  name="describe">{{$memslevel->describe}}</textarea>
			</div>
		</div>		
		<div class="row cl">
...
$(function(){
    ...
    $('#form-admin-role-add').submit(function(evt){
		...
		$.ajax({
			url:'{{url("admin/memslevel/edit")}}'+'/'+"{{$memslevel->level_id}}",
			data:shuju,
			dataType:'json',
			type:'post',
			success:function(msg){               
				if(msg.success===true){
					layer.msg('修改数据成功',{icon:1,time:1000},function(){                        
